...
async def protect_inventory_responses(request: Request, call_next):
    ...
    if path in {"/", "/domains", "/history"} or (
        path.startswith("/domains/") and path.endswith("/comparison")
    ):
...
def home() -> str:
    return (
        HTML.replace("{{DOCUMENTATION_LINK}}", DOCUMENTATION_LINK)
        .replace("{{MAIN_CONTENT}}", HOME_CONTENT)
        .replace("{{PAGE_TITLE}}", "Parralax-DNS")
        .replace("{{PAGE_EYEBROW}}", "Vue d’ensemble")
        .replace("{{PAGE_HEADING}}", "Parralax-DNS")
        .replace("{{PAGE_SUBTITLE}}", "Pilotez les synchronisations et accédez aux outils DNS.")
    )
...
def inventory_page() -> str:
    return (
        HTML.replace("{{DOCUMENTATION_LINK}}", DOCUMENTATION_LINK)
        .replace("{{MAIN_CONTENT}}", INVENTORY_CONTENT)
        .replace("{{PAGE_TITLE}}", "Domaines · Parralax-DNS")
        .replace("{{PAGE_EYEBROW}}", "Inventaire")
        .replace("{{PAGE_HEADING}}", "Domaines")
        .replace("{{PAGE_SUBTITLE}}", "Recherchez les domaines inventoriés et ouvrez le détail de leurs sources DNS.")
    )
...
def comparison_page(domain_id: int) -> str:
    return (
        COMPARISON_HTML.replace("{{DOCUMENTATION_LINK}}", DOCUMENTATION_LINK)
        .replace("{{DOMAIN_ID}}", str(domain_id))
    )
...
<style>
:root { color-scheme: light; font-family: Inter, ui-sans-serif, system-ui, sans-serif; color: #182231; background: #f4f7fb; --brand:#135fca; --brand-deep:#0d3f8d; --brand-cyan:#45b8c8; }
body { box-sizing:border-box;margin:0;min-height:100vh;padding:48px 0;background:radial-gradient(circle at 12% 5%,#dceafe 0,transparent 27rem),radial-gradient(circle at 92% 88%,#dff7f5 0,transparent 30rem),linear-gradient(145deg,#f8faff 0%,#f3f7fb 48%,#eef5f6 100%); }
body::before,body::after{content:"";position:fixed;border-radius:50%;pointer-events:none;z-index:0}body::before{width:32rem;height:32rem;left:-18rem;top:18%;border:1px solid #135fca18;box-shadow:0 0 0 5rem #135fca08,0 0 0 10rem #45b8c807}body::after{width:24rem;height:24rem;right:-12rem;bottom:-8rem;border:1px solid #45b8c824;box-shadow:0 0 0 4rem #45b8c80b,0 0 0 8rem #135fca08}
...
.page-header{display:flex;justify-content:space-between;gap:24px;align-items:flex-start}.brand-title{display:flex;align-items:center;gap:16px}.brand-mark{width:58px;height:58px;flex:0 0 auto;filter:drop-shadow(0 8px 14px #135fca22)}.brand-mark .orbit-a{stroke:var(--brand);fill:#135fca0b}.brand-mark .orbit-b{stroke:var(--brand-cyan);fill:#45b8c810}.brand-mark .axis{stroke:var(--brand-deep)}.brand-mark .node{fill:var(--brand)}.eyebrow{text-transform:uppercase;letter-spacing:.12em;font-size:.72rem;font-weight:750;color:#135fca;margin:0 0 8px}.secondary{background:#fff;color:#135fca;border:1px solid #cdd8e5}.tools{position:relative}.tools summary{list-style:none;cursor:pointer;white-space:nowrap}.tools summary::-webkit-details-marker{display:none}.tools summary::after{content:' ▾'}.tools-popover{position:absolute;right:0;top:48px;width:220px;padding:8px;background:#fff;border:1px solid #dce5ef;border-radius:12px;box-shadow:0 12px 36px #18223120;z-index:2}.tools-popover button,.tools-popover a{display:block;width:100%;box-sizing:border-box;text-align:left;border:0;border-radius:6px;background:white;color:#314154;padding:12px;font:inherit;text-decoration:none}.tools-popover button:hover,.tools-popover a:hover{background:#f1f5fb}.inventory-head{display:flex;align-items:center;justify-content:space-between;gap:16px;margin:26px 0 14px}.inventory-head h2{font-size:1.1rem;margin:0}.inventory-controls{display:flex;align-items:center;justify-content:flex-end;gap:10px;flex-wrap:wrap}.inventory-controls input[type="search"],.inventory-controls select{box-sizing:border-box;padding:9px 10px;border:1px solid #b9c7d7;border-radius:8px;background:#fff;color:#182231;font:inherit}.inventory-controls input[type="search"]{min-width:240px}.archive-toggle{display:inline-flex;align-items:center;gap:7px;white-space:nowrap}.archive-toggle input{width:16px;height:16px;min-width:0;margin:0;accent-color:#135fca}.toolbar{margin-bottom:8px}.sub{max-width:650px;line-height:1.6}.actions{min-width:150px}.actions button{padding:7px 12px}.action-hint{display:block;font-size:.75rem;margin-top:7px;max-width:170px;color:#637083}.table-scroll{overflow-x:auto}.empty h3{color:#182231;margin-top:0}.empty p{line-height:1.6}.empty button{margin-top:8px}.home-intro{position:relative;display:grid;grid-template-columns:minmax(0,1.2fr) minmax(260px,.8fr);gap:32px;align-items:center;margin-top:32px;padding:40px;border:1px solid #dce5ef;border-radius:24px;background:#ffffffd9;box-shadow:0 24px 70px #183a6912;overflow:hidden}.home-intro h2{font-size:1.5rem;margin:0 0 10px}.home-intro p{color:#526274;line-height:1.7;margin:0 0 20px}.depth-stack{position:relative;height:210px}.depth-layer{position:absolute;inset:22px 14px 14px 42px;border:1px solid #135fca28;border-radius:24px;background:linear-gradient(145deg,#ffffffd9,#ddecffb8);box-shadow:0 18px 40px #135fca15}.depth-layer:nth-child(2){inset:42px 34px 30px 22px;transform:translate(16px,-12px);border-color:#45b8c84a;background:linear-gradient(145deg,#f5ffffe8,#dff5f4c7)}.depth-layer:nth-child(3){inset:65px 58px 48px 3px;transform:translate(31px,-23px);display:grid;place-items:center;background:#fff;border-color:#135fca35}.depth-layer svg{width:74px;height:74px}#message:empty{display:none}#message{display:block;margin:12px 0}dialog{box-sizing:border-box;border:1px solid #dce5ef;border-radius:18px;padding:28px;width:min(860px,calc(100% - 32px));max-height:90vh;overflow:auto;color:#182231;box-shadow:0 24px 80px #18223130}dialog::backdrop{background:#14223980}dialog h2{font-size:1.4rem;margin:4px 0 12px}.dialog-head{display:flex;justify-content:space-between;gap:20px;align-items:flex-start}.dialog-head{position:sticky;top:-28px;background:#fff;z-index:1;padding:12px 0}.dialog-head .close{float:none}.connectivity{border:0;padding:16px 0 0;margin:0}.intro{color:#526274;line-height:1.6}.setup-grid{display:grid;grid-template-columns:1fr 1fr;gap:16px}.field{display:flex;flex-direction:column;gap:8px;font-weight:650}.field input,.field select{box-sizing:border-box;width:100%;padding:11px;border:1px solid #b9c7d7;border-radius:8px;background:white;color:#182231;font:inherit}.step{border-top:1px solid #e1e7ef;margin-top:24px;padding-top:20px}.step h3{font-size:1rem;margin:0 0 12px}.step p,.step li{font-size:.9rem;line-height:1.65}.step code{overflow-wrap:anywhere}.code-box{background:#f3f6fa;border-radius:8px;padding:14px;white-space:pre-wrap;overflow-wrap:anywhere;font-size:.8rem;max-height:240px;overflow:auto}.copy-row{display:flex;align-items:center;gap:12px;flex-wrap:wrap}.copy-row button{padding:7px 12px}.callout{padding:12px 16px;background:#edf4ff;border-radius:8px;color:#304e76;font-size:.88rem;line-height:1.6}.setup-status{color:#526274;font-size:.85rem;min-height:1.4em}.steps-overview{font-size:.85rem;color:#526274;padding:12px 0;border-bottom:1px solid #e1e7ef}
@media(max-width:640px){body{padding:24px 0}main{padding:0 16px}h1{font-size:1.65rem}.page-header{gap:8px}.brand-title{gap:10px}.brand-mark{width:46px;height:46px}.home-intro{grid-template-columns:1fr;padding:24px}.depth-stack{height:160px}.setup-grid{grid-template-columns:1fr}dialog{padding:20px}.dialog-head{top:-20px}.inventory-head{align-items:flex-start;flex-direction:column}.inventory-controls{justify-content:flex-start}.inventory-controls input[type="search"]{min-width:0;width:100%}.toolbar>button{flex:1}.nav-link{padding:9px 10px}th,td{padding:12px 10px}}
</style></head><body><main>
<header class="page-header"><div class="brand-title"><svg class="brand-mark" viewBox="0 0 64 64" aria-hidden="true"><circle class="orbit-a" cx="27" cy="30" r="20" stroke-width="2"/><circle class="orbit-b" cx="38" cy="34" r="17" stroke-width="2"/><path class="axis" d="M13 42L49 19" stroke-width="2" stroke-linecap="round"/><circle class="node" cx="21" cy="37" r="4"/><circle class="node" cx="43" cy="23" r="4"/></svg><div><p class="eyebrow">{{PAGE_EYEBROW}}</p><h1>{{PAGE_HEADING}}</h1><p class="sub">{{PAGE_SUBTITLE}}</p></div></div>
<details class="tools" id="tools"><summary class="nav-link">Outils</summary><nav class="tools-popover" aria-label="Outils"><a href="/domains">Domaines</a><button id="open-schedule">Planification des synchronisations</button><button id="open-diagnostics">Diagnostics API</button><button onclick="openSetup()">Connecter un serveur</button><a href="/history">Historique</a>{{DOCUMENTATION_LINK}}</nav></details></header>
<div class="toolbar"><a class="nav-link" href="/domains">Voir les domaines</a><button onclick="openSetup()">Connecter un serveur</button><button class="secondary" id="sync">Synchroniser les API</button><a class="nav-link" href="/history">Historique</a></div>
<p class="notice">Les serveurs connectés envoient leurs données depuis leur propre collecteur.</p><p id="message" role="status"></p>
{{MAIN_CONTENT}}
<dialog id="schedule-dialog" aria-labelledby="schedule-title"><header class="dialog-head"><div><p class="eyebrow">Outils</p><h2 id="schedule-title">Planification des synchronisations</h2></div><button class="close" onclick="q('#schedule-dialog').close()">Fermer</button></header><section id="schedule" class="schedule-panel" aria-live="polite"><p class="muted">Chargement de la planification…</p></section></dialog>
...
<p class="intro">Un collecteur installé sur votre serveur ou cluster lit son inventaire DNS et l’envoie à Parralax-DNS. Les collecteurs Linux fournis ciblent Plesk ou Kubernetes.</p>
<p class="steps-overview">1. Créer le jeton → 2. L’activer dans Parralax-DNS → 3. Installer le collecteur → 4. Vérifier la collecte</p>
<form id="setup-form"><div class="setup-grid"><label class="field">Type de serveur<select id="collector-kind"><option value="plesk">shell-child</option><option value="kubernetes">Kubernetes (Ingress)</option><option value="windows">Windows DNS</option></select></label><label class="field" id="source-field">Identifiant de la source<input id="collector-source" value="shell-00X" required maxlength="128" pattern="[A-Za-z0-9][A-Za-z0-9._\-]{0,127}" aria-describedby="source-help"></label></div>
<p class="notice" id="source-help">Choisissez un identifiant unique par serveur, sans espaces (ex. shell-00X).</p>
<p><label class="field">URL de Parralax-DNS<input id="collector-url" type="url" placeholder="https://dns.votre-entreprise.fr ou http://172.20.205.197:1337" required aria-describedby="url-help"></label></p><p class="notice" id="url-help">L’adresse HTTP ou HTTPS de cette application, accessible depuis le serveur à connecter.</p>
<section class="step"><h3>1. Créer un jeton de collecte</h3><p>Ce jeton autorise l’envoi vers Parralax-DNS. Aucune clé API tierce n’est nécessaire.</p><button type="submit" id="generate-token">Générer le jeton et les instructions</button><p class="setup-status" id="setup-status" role="status"></p></section></form>
...
<section class="step"><h3>4. Vérifier la première collecte</h3><p id="verify-help"></p><p>Une réponse <code>status: success</code> confirme la réception. Fermez cet assistant, puis actualisez la liste. Planifiez ensuite le collecteur avec cron sous Linux ou le Planificateur de tâches sous Windows.</p><button class="secondary" id="refresh-domains">Actualiser les domaines</button><details><summary>La collecte échoue ?</summary><ul><li><strong>401 :</strong> vérifiez que le jeton est identique des deux côtés et que l’identifiant de source correspond.</li><li><strong>503 :</strong> vérifiez la variable côté Parralax-DNS et recréez ou redémarrez le service.</li><li><strong>Connexion impossible :</strong> vérifiez l’URL HTTP ou HTTPS, le certificat le cas échéant et l’accès réseau depuis le serveur source.</li></ul></details></section>
...
let inventory=[];
function comparisonAction(d){return '<a class="nav-link" href="/history?domain='+d.id+'">Voir le détail</a>'}
const searchText=value=>String(value??'').normalize('NFD').replace(/[\u0300-\u036f]/g,'').toLocaleLowerCase('fr-FR');
function renderInventory(){const query=searchText(q('#domain-search').value.trim());const sort=q('#domain-sort').value;const domains=inventory.filter(d=>!query||searchText([d.name,...d.sources.map(s=>s.provider+' '+(s.remote_status||''))].join(' ')).includes(query));domains.sort((a,b)=>{if(sort==='name-desc')return String(b.name).localeCompare(String(a.name),'fr',{numeric:true,sensitivity:'base'});if(sort==='last-seen-desc')return (Date.parse(b.last_seen_at)||0)-(Date.parse(a.last_seen_at)||0);if(sort==='sources-desc')return b.sources.length-a.sources.length||String(a.name).localeCompare(String(b.name),'fr',{numeric:true,sensitivity:'base'});return String(a.name).localeCompare(String(b.name),'fr',{numeric:true,sensitivity:'base'})});q('#domain-count').textContent=inventory.length===domains.length?'('+inventory.length+')':'('+domains.length+' / '+inventory.length+')';if(!domains.length){const message=inventory.length?'Aucun domaine ne correspond à votre recherche.':(q('#archived').checked?'Aucun domaine pour le moment':'Aucun domaine actif');q('#content').innerHTML='<div class="empty"><h3>'+message+'</h3>'+(inventory.length?'':'<p>Connectez un serveur avec l’assistant.<br>Si vous avez déjà configuré un fournisseur API, lancez une synchronisation.</p><button onclick="openSetup()">Connecter mon premier serveur</button>')+'</div>';return}q('#content').innerHTML='<table><thead><tr><th>Domaine</th><th>Sources</th><th>État</th><th>Dernière collecte</th><th>Actions</th></tr></thead><tbody>'+domains.map(d=>'<tr><td><strong>'+esc(d.name)+'</strong></td><td>'+d.sources.map(s=>'<div class="source"><strong>'+esc(s.provider)+'</strong> <span class="muted">'+esc(s.remote_status||'—')+'</span>'+(s.provider==='cloudflare'?'<br><a href="/api/sources/'+s.id+'/zone-file">Télécharger BIND</a> · <a href="#" onclick="cloneZone('+s.id+');return false">Cloner vers Infomaniak</a>':'')+'</div>').join('')+'</td><td><span class="badge '+(d.lifecycle_status==='archived'?'archived':'')+'">'+(d.lifecycle_status==='archived'?'Archivé':'Actif')+'</span></td><td class="muted">'+esc(dateTime(d.last_seen_at))+'</td><td class="actions">'+comparisonAction(d)+'</td></tr>').join('')+'</tbody></table>'}
async function load(){try{const r=await fetch('/api/domains?include_archived='+q('#archived').checked);if(!r.ok)throw Error('Chargement indisponible');inventory=await r.json();renderInventory()}catch(e){q('#content').innerHTML='<div class="empty warning">Impossible de charger les domaines. <button onclick="load()">Réessayer</button></div>'}}
...
async function loadSchedule(){const panel=q('#schedule');panel.innerHTML='<p class="muted">Chargement de la planification…</p>';try{const r=await fetch('/api/sync-schedule');const data=await r.json();if(!r.ok)throw Error(data.detail);if(!data.enabled){panel.innerHTML='<p class="muted">La synchronisation automatique est désactivée. Définissez <code>SYNC_INTERVAL_MINUTES</code> dans l’environnement puis redémarrez le service.</p>';return}const cadence=data.interval_minutes?'Toutes les '+esc(data.interval_minutes)+' minute(s).':'Au démarrage uniquement.';const next=data.next_run_at?' Prochaine exécution : '+esc(dateTime(data.next_run_at))+'.':'';const last=data.last_completed_at?' Dernier résultat : '+esc(data.last_status||'inconnu')+' · '+esc(dateTime(data.last_completed_at))+'.':' En attente de la première exécution.';panel.innerHTML='<p><span class="badge '+(data.running?'status-pending':'')+'">'+(data.running?'En cours':'Planifiée')+'</span> '+cadence+next+last+'</p><p class="notice">Les collecteurs Windows DNS et les sources personnalisées sont planifiés depuis leurs serveurs source.</p>'}catch(e){panel.innerHTML='<p class="warning">Impossible de lire la planification : '+esc(e.message)+'</p>'}}
...
q('#open-schedule').onclick=()=>{q('#tools').open=false;q('#schedule-dialog').showModal();loadSchedule()};
...
q('#setup-form').onsubmit=e=>{
  e.preventDefault();
  let base;
  try{const url=new URL(q('#collector-url').value);if(!['http:','https:'].includes(url.protocol)||url.username||url.password||url.search||url.hash)throw Error();base=url.href.replace(/\/$/,'')}
  catch{q('#collector-url').setCustomValidity('Utilisez une URL HTTP ou HTTPS sans identifiants, paramètres ni fragment.');q('#collector-url').reportValidity();return}
  ...
  const kind=q('#collector-kind').value;const windows=kind==='windows';const kubernetes=kind==='kubernetes';const source=q('#collector-source').value.trim().toLowerCase();
  ...
  q('#collector-help').textContent=windows?'Avec Windows PowerShell 5.1 et le module DnsServer, placez le script dans C:\\Program Files\\Parralax-DNS. Créez C:\\ProgramData\\Parralax-DNS, puis enregistrez ce JSON dans Sync-ParralaxDns.json à cet emplacement.':kubernetes?'Dans le cluster, exécutez le manifeste fourni. Il crée un CronJob en lecture seule qui lit les Ingress de tous les espaces de noms ; le jeton est stocké dans un Secret Kubernetes.':'Sur le serveur Plesk Linux, installez bash, jq et curl, puis placez le script dans /opt/parralax-plesk-collector/collect-plesk-dns.sh. Créez /etc/parralax-plesk-collector.env avec ce contenu et limitez ses permissions à 0600.';
  ...
  q('#run-help').textContent=windows?'Lancez cette commande dans une console PowerShell administrateur sur le serveur DNS :':kubernetes?'Créez le Secret avec les trois variables ci-dessus, appliquez le manifeste Kubernetes, puis lancez une exécution de contrôle :':'Depuis le serveur Plesk, avec un compte autorisé à lire toutes les zones, vérifiez d’abord la collecte sans envoi :';
  ...
  q('#verify-help').textContent=windows?'La commande précédente envoie les zones du serveur à Parralax-DNS.':kubernetes?'Consultez les logs du Job ; il envoie les hôtes d’Ingress et les adresses de load balancer observées à Parralax-DNS.':'Vérifiez le JSON obtenu, puis relancez /opt/parralax-plesk-collector/collect-plesk-dns.sh sans --dry-run pour envoyer les données.';
...
q('#refresh-domains').onclick=()=>{q('#setup-dialog').close();if(q('#content'))load();else location.href='/domains'};

q('#sync').onclick=async()=>{const b=q('#sync');b.disabled=true;q('#message').textContent='Synchronisation en cours…';try{const r=await fetch('/api/sync',{method:'POST',headers:uiWriteHeaders});const data=await r.json();if(!r.ok)throw Error(data.detail);q('#message').textContent=data.results.map(x=>x.provider+': '+x.status+(x.discovered!==undefined?' ('+x.discovered+' trouvés)':'')).join(' · ');if(q('#content'))await load();if(q('#schedule-dialog').open)await loadSchedule()}catch(e){q('#message').textContent='Erreur : '+e.message}finally{b.disabled=false}};q('#diagnostics').onclick=testAllConnectivity;if(q('#content')){q('#archived').onchange=load;q('#domain-search').oninput=renderInventory;q('#domain-sort').onchange=renderInventory;load()}
</script></body></html>'''


COMPARISON_HTML = r'''<!doctype html>
<html lang="fr"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1">
<title>Comparateur DNS · Parralax-DNS</title>
<style>
:root{color-scheme:light;font-family:Inter,ui-sans-serif,system-ui,sans-serif;color:#182231;background:#f4f7fb;--brand:#135fca;--brand-deep:#0d3f8d;--brand-cyan:#45b8c8}
body{box-sizing:border-box;margin:0;min-height:100vh;padding:48px 0;background:radial-gradient(circle at 12% 5%,#dceafe 0,transparent 27rem),radial-gradient(circle at 92% 88%,#dff7f5 0,transparent 30rem),linear-gradient(145deg,#f8faff 0%,#f3f7fb 48%,#eef5f6 100%)}
body::before,body::after{content:"";position:fixed;border-radius:50%;pointer-events:none;z-index:0}body::before{width:32rem;height:32rem;left:-18rem;top:18%;border:1px solid #135fca18;box-shadow:0 0 0 5rem #135fca08,0 0 0 10rem #45b8c807}body::after{width:24rem;height:24rem;right:-12rem;bottom:-8rem;border:1px solid #45b8c824;box-shadow:0 0 0 4rem #45b8c80b,0 0 0 8rem #135fca08}
main{position:relative;z-index:1;max-width:1200px;margin:0 auto;padding:0 24px}h1{margin:0;font-size:2rem}.sub{color:#637083;line-height:1.6;margin:8px 0 24px;max-width:760px}.page-header{display:flex;align-items:flex-start;margin-bottom:22px}.brand-title{display:flex;align-items:center;gap:16px}.brand-mark{width:58px;height:58px;flex:0 0 auto;filter:drop-shadow(0 8px 14px #135fca22)}.brand-mark .orbit-a{stroke:var(--brand);fill:#135fca0b}.brand-mark .orbit-b{stroke:var(--brand-cyan);fill:#45b8c810}.brand-mark .axis{stroke:var(--brand-deep)}.brand-mark .node{fill:var(--brand)}.eyebrow{text-transform:uppercase;letter-spacing:.12em;font-size:.72rem;font-weight:750;color:var(--brand);margin:0 0 8px}
.toolbar,.picker-actions,.result-tools{display:flex;align-items:center;gap:10px;flex-wrap:wrap}.toolbar{margin-bottom:20px}.nav-link{display:inline-block;border:1px solid #cdd8e5;border-radius:8px;padding:9px 14px;background:#fff;color:#135fca;font-weight:650;text-decoration:none}button{border:0;border-radius:8px;padding:10px 16px;background:#135fca;color:#fff;font-weight:650;cursor:pointer}button:disabled{background:#e8edf3;color:#687587;cursor:not-allowed}.secondary{background:#fff;color:#135fca;border:1px solid #cdd8e5}.panel{padding:22px;background:#fff;border:1px solid #dce5ef;border-radius:14px;box-shadow:0 18px 54px #183a690c}.panel h2{font-size:1.15rem;margin:0 0 6px}.muted{color:#637083;font-size:.88rem}.error{color:#a33d10}.source-picker{display:grid;grid-template-columns:repeat(auto-fit,minmax(240px,1fr));gap:8px;margin:16px 0}.source-picker label{display:flex;align-items:flex-start;gap:9px;padding:11px 12px;border:1px solid #dce5ef;border-radius:8px;background:#f9fbfe;font-size:.88rem;line-height:1.35}.source-picker input{margin-top:2px}.picker-actions button{padding:7px 11px;font-size:.82rem}.result{margin-top:20px}.summary{padding:14px 16px;border-radius:9px;background:#edf4ff;color:#304e76}.result-tools{margin:18px 0 10px}.result-tools select{padding:8px 11px;border:1px solid #cdd8e5;border-radius:8px;background:#fff;color:#182231;font:inherit}.record-set{padding:12px 0;border-top:1px solid #e4eaf1}.record-set summary{cursor:pointer;line-height:1.5}.record-set .error{margin-bottom:8px}.table-scroll{overflow-x:auto;margin-top:10px}table{min-width:680px;width:100%;border-collapse:collapse;background:#fff;border:1px solid #e0e6ed}th,td{padding:11px 12px;border-bottom:1px solid #e9edf2;text-align:left;vertical-align:top}th{color:#556577;font-size:.76rem;text-transform:uppercase;letter-spacing:.04em;background:#fafbfd}td{font-family:ui-monospace,SFMono-Regular,monospace;font-size:.8rem;word-break:break-word}.empty{padding:24px;border:1px dashed #c8d1dc;border-radius:10px;color:#637083}.errors{padding:14px 18px;background:#fff5f0;border-radius:9px}.errors li{margin:5px 0}
[hidden]{display:none!important}button:focus-visible,a:focus-visible,input:focus-visible,select:focus-visible,summary:focus-visible{outline:3px solid #79aaf1;outline-offset:3px}@media(max-width:640px){body{padding:24px 0}main{padding:0 16px}h1{font-size:1.65rem}.brand-title{gap:10px}.brand-mark{width:46px;height:46px}.panel{padding:17px}.source-picker{grid-template-columns:1fr}}
</style></head><body><main>
<header class="page-header"><div class="brand-title"><svg class="brand-mark" viewBox="0 0 64 64" aria-hidden="true"><circle class="orbit-a" cx="27" cy="30" r="20" stroke-width="2"/><circle class="orbit-b" cx="38" cy="34" r="17" stroke-width="2"/><path class="axis" d="M13 42L49 19" stroke-width="2" stroke-linecap="round"/><circle class="node" cx="21" cy="37" r="4"/><circle class="node" cx="43" cy="23" r="4"/></svg><div><p class="eyebrow">Analyse multi-sources</p><h1 id="page-title">Comparateur DNS</h1><p class="sub">Comparez les cibles, les valeurs et les paramètres DNS d’un même domaine entre plusieurs providers.</p></div></div></header>
<nav class="toolbar" aria-label="Navigation"><a id="detail-link" class="nav-link" href="/history?domain={{DOMAIN_ID}}">← Retour au détail</a><a class="nav-link" href="/domains">Domaines</a><a class="nav-link" href="/">Accueil</a>{{DOCUMENTATION_LINK}}</nav>
<section class="panel" aria-live="polite"><h2>Sources à comparer</h2><p id="source-help" class="muted">Chargement des sources du domaine…</p><div id="picker-actions" class="picker-actions" hidden><button class="secondary" type="button" onclick="toggleSources(true)">Tout sélectionner</button><button class="secondary" type="button" onclick="toggleSources(false)">Tout désélectionner</button></div><div id="source-picker" class="source-picker"></div><button id="compare-button" type="button" onclick="compareSelected()" disabled>Comparer les sources sélectionnées</button><section id="comparison-result" class="result"></section></section>
</main><script>
const domainId={{DOMAIN_ID}};const q=s=>document.querySelector(s);const esc=s=>String(s??'').replace(/[&<>'"]/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;',"'":'&#39;','"':'&quot;'}[c]));let latestRecordSets=[];
const comparisonValue=values=>values?.length?values.map(value=>typeof value==='string'?value:JSON.stringify(value)).join(', '):'—';
function toggleSources(checked){document.querySelectorAll('input[name="comparison-source"]').forEach(input=>input.checked=checked);updateCompareButton()}
function updateCompareButton(){const selected=document.querySelectorAll('input[name="comparison-source"]:checked').length;const button=q('#compare-button');button.disabled=selected<2;button.textContent=selected<2?'Sélectionnez au moins deux sources':'Comparer '+selected+' sources'}
function recordSetDetail(recordSet){const sources=Object.keys(recordSet.sources);const status={different:'Différent',missing:'Absent sur une source'}[recordSet.status]||'Identique';const rows=recordSet.field_differences.map(difference=>'<tr><th>'+esc(difference.label)+'</th>'+sources.map(source=>'<td>'+esc(comparisonValue(difference.values[source]))+'</td>').join('')+'</tr>').join('');const missing=recordSet.missing_from.length?'<p class="error">Absent chez : '+esc(recordSet.missing_from.join(', '))+'</p>':'';return '<details class="record-set"><summary><strong>'+esc(recordSet.name)+' · '+esc(recordSet.type)+'</strong> — '+esc(status)+'</summary>'+missing+'<div class="table-scroll"><table><thead><tr><th>Champ</th>'+sources.map(source=>'<th>'+esc(source)+'</th>').join('')+'</tr></thead><tbody>'+rows+'</tbody></table></div></details>'}
function renderRecordSets(){const type=q('#record-type')?.value||'';const visible=latestRecordSets.filter(recordSet=>recordSet.status!=='identical'&&(!type||recordSet.type===type));q('#record-sets').innerHTML=visible.length?visible.map(recordSetDetail).join(''):'<div class="empty">Aucun écart pour ce type d’enregistrement.</div>'}
function renderComparison(data){latestRecordSets=data.dns.record_sets||[];const summary=data.dns.record_set_summary||{};const types=[...new Set(latestRecordSets.map(recordSet=>recordSet.type))].sort();const errors=Object.entries(data.dns.errors||{}).map(([source,error])=>'<li><strong>'+esc(source)+'</strong> : '+esc(error)+'</li>').join('');q('#comparison-result').innerHTML='<p class="summary"><strong>'+esc(data.dns.sources_read.length)+' source(s) lue(s)</strong> · '+esc(summary.different||0)+' différence(s) · '+esc(summary.missing||0)+' absence(s) · '+esc(summary.identical||0)+' ensemble(s) identique(s)</p><div class="result-tools"><label>Type d’enregistrement <select id="record-type"><option value="">Tous les types</option>'+types.map(type=>'<option value="'+esc(type)+'">'+esc(type)+'</option>').join('')+'</select></label></div><p class="muted">Les écarts sont regroupés par nom et type : cible pour CNAME, MX, NS, PTR et SRV ; valeur pour A, AAAA, TXT et CAA ; puis paramètres DNS applicables.</p><h2>Écarts détaillés</h2><div id="record-sets"></div>'+(errors?'<h2 class="error">Sources indisponibles</h2><ul class="errors">'+errors+'</ul>':'');q('#record-type').addEventListener('change',renderRecordSets);renderRecordSets()}
async function compareSelected(){const selected=[...document.querySelectorAll('input[name="comparison-source"]:checked')].map(input=>input.value);if(selected.length<2){updateCompareButton();return}const panel=q('#comparison-result');const button=q('#compare-button');button.disabled=true;panel.innerHTML='<p class="muted">Analyse de '+esc(selected.length)+' sources en cours…</p>';try{const query='?'+selected.map(sourceId=>'source_ids='+encodeURIComponent(sourceId)).join('&');const response=await fetch('/api/domains/'+domainId+'/comparison'+query);const data=await response.json();if(!response.ok)throw Error(data.detail||'Erreur inconnue');renderComparison(data)}catch(error){panel.innerHTML='<div class="empty error">Impossible de comparer les sources : '+esc(error.message)+'</div>'}finally{updateCompareButton()}}
async function loadDomain(){try{const response=await fetch('/api/domains/'+domainId);const domain=await response.json();if(!response.ok)throw Error(domain.detail||'Domaine introuvable');document.title='Comparateur · '+domain.name+' · Parralax-DNS';q('#page-title').textContent='Comparateur · '+domain.name;const sources=domain.sources||[];q('#source-help').textContent=sources.length+' source(s) disponible(s). Toutes sont sélectionnées par défaut.';q('#source-picker').innerHTML=sources.map(source=>'<label><input type="checkbox" name="comparison-source" value="'+esc(source.id)+'" checked> <span><strong>'+esc(source.provider)+'</strong><br><span class="muted">'+esc(source.external_id)+'</span></span></label>').join('');document.querySelectorAll('input[name="comparison-source"]').forEach(input=>input.addEventListener('change',updateCompareButton));if(sources.length<2){q('#source-help').textContent='La comparaison nécessite au moins deux sources pour ce domaine.';q('#comparison-result').innerHTML='<div class="empty">Ajoutez une seconde source à ce domaine pour lancer une comparaison.</div>';return}q('#picker-actions').hidden=false;updateCompareButton();compareSelected()}catch(error){q('#source-help').textContent='Impossible de charger le domaine.';q('#comparison-result').innerHTML='<div class="empty error">'+esc(error.message)+'</div>'}}
loadDomain();
</script></body></html>'''


HOME_CONTENT = r'''<section class="home-intro"><div><p class="eyebrow">Une vue, plusieurs profondeurs</p><h2>Votre infrastructure DNS mise en perspective</h2><p>Parralax-DNS rapproche les observations de chaque provider pour faire ressortir les écarts, sans surcharger votre vue d’ensemble.</p><a class="nav-link" href="/domains">Ouvrir l’inventaire des domaines</a></div><div class="depth-stack" aria-hidden="true"><div class="depth-layer"></div><div class="depth-layer"></div><div class="depth-layer"><svg class="brand-mark" viewBox="0 0 64 64"><circle class="orbit-a" cx="27" cy="30" r="20" stroke-width="2"/><circle class="orbit-b" cx="38" cy="34" r="17" stroke-width="2"/><path class="axis" d="M13 42L49 19" stroke-width="2" stroke-linecap="round"/><circle class="node" cx="21" cy="37" r="4"/><circle class="node" cx="43" cy="23" r="4"/></svg></div></div></section>'''


INVENTORY_CONTENT = r'''<div class="inventory-head"><h2>Domaines <span class="muted" id="domain-count"></span></h2><div class="inventory-controls"><label class="archive-toggle"><input type="checkbox" id="archived"> <span>Afficher les archivés</span></label><label><span class="muted">Trier par </span><select id="domain-sort" aria-label="Trier les domaines"><option value="name-asc">Nom, A → Z</option><option value="name-desc">Nom, Z → A</option><option value="last-seen-desc">Dernière collecte</option><option value="sources-desc">Nombre de sources</option></select></label><label><input id="domain-search" type="search" placeholder="Rechercher un domaine ou une source" aria-label="Rechercher un domaine ou une source"></label></div></div>
<div id="content" class="table-scroll" aria-live="polite"><div class="empty">Chargement des domaines…</div></div>'''
...
</style><style>:root{--brand:#135fca;--brand-deep:#0d3f8d;--brand-cyan:#45b8c8}body{box-sizing:border-box;min-height:100vh;padding:48px 0;background:radial-gradient(circle at 12% 5%,#dceafe 0,transparent 27rem),radial-gradient(circle at 92% 88%,#dff7f5 0,transparent 30rem),linear-gradient(145deg,#f8faff 0%,#f3f7fb 48%,#eef5f6 100%)}body::before,body::after{content:"";position:fixed;border-radius:50%;pointer-events:none;z-index:0}body::before{width:32rem;height:32rem;left:-18rem;top:18%;border:1px solid #135fca18;box-shadow:0 0 0 5rem #135fca08,0 0 0 10rem #45b8c807}body::after{width:24rem;height:24rem;right:-12rem;bottom:-8rem;border:1px solid #45b8c824;box-shadow:0 0 0 4rem #45b8c80b,0 0 0 8rem #135fca08}main{position:relative;z-index:1;margin:0 auto}.page-header{display:flex;align-items:flex-start;margin-bottom:22px}.brand-title{display:flex;align-items:center;gap:16px}.brand-mark{width:58px;height:58px;flex:0 0 auto;filter:drop-shadow(0 8px 14px #135fca22)}.brand-mark .orbit-a{stroke:var(--brand);fill:#135fca0b}.brand-mark .orbit-b{stroke:var(--brand-cyan);fill:#45b8c810}.brand-mark .axis{stroke:var(--brand-deep)}.brand-mark .node{fill:var(--brand)}.eyebrow{text-transform:uppercase;letter-spacing:.12em;font-size:.72rem;font-weight:750;color:var(--brand);margin:0 0 8px}.detail-actions{display:flex;align-items:center;gap:10px;flex-wrap:wrap;margin:16px 0 4px}.danger{background:#a33d10}.domain-action-status{min-height:1.3em;margin:10px 0 0}@media(max-width:640px){body{padding:24px 0}.brand-title{gap:10px}.brand-mark{width:46px;height:46px}}</style></head><body><main>
<header class="page-header"><div class="brand-title"><svg class="brand-mark" viewBox="0 0 64 64" aria-hidden="true"><circle class="orbit-a" cx="27" cy="30" r="20" stroke-width="2"/><circle class="orbit-b" cx="38" cy="34" r="17" stroke-width="2"/><path class="axis" d="M13 42L49 19" stroke-width="2" stroke-linecap="round"/><circle class="node" cx="21" cy="37" r="4"/><circle class="node" cx="43" cy="23" r="4"/></svg><div><p class="eyebrow">Journal DNS</p><h1>Historique</h1><p class="sub">Découvertes, mises à jour, archivages, restaurations et suivi des sources.</p></div></div></header>
<div class="toolbar"><a class="nav-link" href="/domains">← Inventaire</a><a class="nav-link" href="/">Accueil</a>{{DOCUMENTATION_LINK}}<input id="filter" class="filter" type="search" placeholder="Filtrer par domaine ou source"></div>
...
const q=s=>document.querySelector(s);const esc=s=>String(s??'').replace(/[&<>'"]/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;',"'":'&#39;','"':'&quot;'}[c]));const fmt=v=>v?new Date(v).toLocaleString('fr-FR',{dateStyle:'medium',timeStyle:'medium'}):'—';const uiWriteHeaders={'X-Parralax-UI-Request':'1'};let entries=[];let currentDomainName='';const domainParam=new URLSearchParams(location.search).get('domain');
...
function renderDomainDetail(domain){const panel=q('#domain-detail');const sources=domain.sources||[];currentDomainName=domain.name;const comparison=sources.length>=2?'<a class="nav-link" href="/domains/'+encodeURIComponent(domain.id)+'/comparison">Comparer les '+esc(sources.length)+' sources</a>':'<span class="muted">La comparaison sera disponible avec au moins deux sources.</span>';const archiveAction=domain.lifecycle_status==='archived'?'<span class="badge archived">Domaine archivé</span>':'<button class="danger" id="archive-domain" type="button" onclick="archiveDomain('+domain.id+')">Archiver ce domaine</button>';panel.hidden=false;panel.innerHTML='<h2>Détail · '+esc(domain.name)+'</h2><p class="muted">Configuration et métadonnées actuelles reçues lors de la dernière synchronisation, pour chaque source du domaine.</p><div class="detail-actions">'+comparison+archiveAction+'</div><p id="domain-action-status" class="domain-action-status muted" role="status"></p><div class="metadata-grid">'+(sources.length?sources.map(source=>sourceCard(source)).join(''):'<p class="muted">Aucune source enregistrée.</p>')+'</div>';sources.forEach(source=>loadSourceSnapshots(source,domain.name))}
async function archiveDomain(id){if(!confirm('Archiver '+currentDomainName+' et toutes ses sources ?\n\nLes données et l’historique seront conservés. Une prochaine synchronisation pourra restaurer le domaine s’il est toujours présent chez un provider.'))return;const button=q('#archive-domain');const status=q('#domain-action-status');button.disabled=true;status.textContent='Archivage en cours…';try{const response=await fetch('/api/domains/'+encodeURIComponent(id)+'/archive',{method:'POST',headers:uiWriteHeaders});const data=await response.json();if(!response.ok)throw Error(data.detail||'Erreur inconnue');await loadDomainDetail();await load();q('#domain-action-status').textContent='Le domaine et ses sources sont archivés. Les données restent consultables.'}catch(error){button.disabled=false;status.textContent='Impossible d’archiver le domaine : '+error.message}}
const snapshotLabel=s=>fmt(s.run_completed_at||s.captured_at)+' · '+esc(s.trigger||'manual');
async function loadSourceSnapshots(source,domainName){const panel=q('#snapshots-'+source.id);try{const r=await fetch('/api/sources/'+source.id+'/snapshots');const data=await r.json();if(!r.ok)throw Error(data.detail||'Erreur inconnue');const snapshots=data.snapshots||[];if(snapshots.length<2){panel.innerHTML='<p class="muted">La comparaison entre versions sera disponible après deux synchronisations de cette source.</p>';return}const options=snapshots.map(snapshot=>'<option value="'+esc(snapshot.sync_run_id)+'">'+snapshotLabel(snapshot)+'</option>').join('');panel.innerHTML='<h4>Comparer deux versions synchronisées</h4><label>Version antérieure <select onchange="updateSnapshotButton('+source.id+')" id="before-'+source.id+'">'+options+'</select></label><label>Version récente <select onchange="updateSnapshotButton('+source.id+')" id="after-'+source.id+'">'+options+'</select></label><button id="compare-snapshots-'+source.id+'" onclick="compareSourceSnapshots('+source.id+',\''+esc(domainName)+'\')">Comparer les versions</button><div id="snapshot-result-'+source.id+'"></div>';q('#before-'+source.id).selectedIndex=1}catch(e){panel.innerHTML='<p class="error">Impossible de charger les synchronisations : '+esc(e.message)+'</p>'}}
